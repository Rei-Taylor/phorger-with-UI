...
def setup():
    ...
    try:
        with open(
            "settings.json", mode="r", encoding="UTF-8"
        ) as setting_file:  # opening settings file
            settings: dict = json.load(
                setting_file
            )  # serializing json file into python readable format
    except (FileNotFoundError, UnboundLocalError):
        print("ERROR: settings file not found or corrupt")
        sys.exit(1)

    if Path(settings["source_filepath"]) == Path(
        settings["destination_filepath"]
...
def mode_selection():
    ...
    if len(sys.argv) == 2 and (
        sys.argv[1] == "arc" or sys.argv[1] == "cld"
    ):  # checks for single and correct argument
        mode = sys.argv[1]
