...
def path_validator(destination: Path, size: int):
    ...
    total, used, free = shutil.disk_usage(
        destination.anchor
    )  # checking for available space on destination drive
...
def duplicate_checker(source_list: list, destination: Path) -> list:
    ...
    if duplicates:
        ...
        files_to_move = [
            file for file in source_list if file not in duplicates
        ]  # adjusting the source file list to exclude duplicates
