...
import os.path
import json, logging
logging.basicConfig(filename="log.txt", filemode='w', level=logging.DEBUG, format='%(message)s')
...
def auth_service():
    ...
    service = build("drive", "v3", credentials=creds) #returns discovery resource object
    return service
...
def folder(service):
    results = ( #checking for exisiting folder
        service
        .files()
        .list(q="mimeType='application/vnd.google-apps.folder'", fields="files(id, name)") #filters out folder
        .execute() 
    )    
    ...
    file_metadata = {"name": "phorger", "mimeType": "application/vnd.google-apps.folder"}
...
def album_folder(service, phorger_folder_id, album_name):
    ...
    file_metadata = {"name": album_name, "mimeType": "application/vnd.google-apps.folder", "parents": [phorger_folder_id]}
    folder = service.files().create(body=file_metadata, fields="id").execute()
...
def file_uploader(service, folder_id, file_list):
    ...
    for path_object in file_list:
        ...
        for file in output_file_path.glob("*.jpg"): #uploading only jpgs found in output folder
            ...
            media = MediaFileUpload(str(file), mimetype="image/jpeg") # converting path object to string

            file = (
                service.files()
                .create(body=file_metadata, media_body=media, fields="id", uploadType="multipart")
                .execute()
            )
            counter += 1
